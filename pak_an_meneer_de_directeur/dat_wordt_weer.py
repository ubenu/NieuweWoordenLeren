...
class Main(QMainWindow, Ui_MainWindow):
    ...
    readiness = [1, 2, 4]
    WORDS_SET, SCORING_SET, PLAYER_SET = readiness
    answers = ['fout', 
               'goed',
               ]
    ...
    def __init__(self, ):
        ...
        self.action_open_personal_score.triggered.connect(self.on_open_personal_score)
        ...
        self.current_readiness = 0
        self.file_name_word_list = None
        self.file_name_scoring = None
        self.file_name_player = None
        self.full_word_list = None
        self.full_player_data = None
        self.full_scoring_info = None
        self.words_in_heaps = {}
        ...
        self.update_game()
        self.update_ui()
        self.on_open_word_list('..\\Resources\\Input\\Woordenlijst.csv')
        self.on_open_scoring('..\\Resources\\Input\\Puntentelling.csv')
        self.on_open_personal_score('..\\Resources\\Input\\Speler.csv')
        self.update_game()
        self.update_ui()
        

    def on_open_word_list(self, file_path):
        if file_path == "":
        ...
        if file_path:
            ...
            self.current_readiness += self.WORDS_SET
            if self.current_readiness < sum(self.readiness):
                self.current_state = 'preparing'
            elif self.current_readiness == sum(self.readiness):
                self.current_state = 'ready'
            self.update_game()
            self.update_ui()
            
    def on_open_personal_score(self, file_path):
        if file_path == "":
            file_path = widgets.QFileDialog.getOpenFileName(self, 
            "Open speler gegevens", "", "CSV data files (*.csv);;All files (*.*)")[0]
        if file_path:
            info = qt.QFileInfo(file_path)
            self.file_name_word_list = info.fileName()
            self.import_player(file_path)
            self.current_readiness += self.PLAYER_SET
            if self.current_readiness < sum(self.readiness):
                self.current_state = 'preparing'
            elif self.current_readiness == sum(self.readiness):
                self.current_state = 'ready'
    ...
    def on_open_scoring(self, file_path):
        if file_path == "":
            file_path = widgets.QFileDialog.getOpenFileName(self, 
        ...
        if file_path:
            ...
            self.current_readiness += self.SCORING_SET
            if self.current_readiness < sum(self.readiness):
                self.current_state = 'preparing'
            elif self.current_readiness == sum(self.readiness):
                self.current_state = 'ready'
    ...
    def import_player(self, file_path): 
        self.full_player_data = pd.read_csv(file_path) 
        print(self.full_player_data)
        status = self.full_player_data['Stapel']
        self.words_in_heaps = {}
        for heap in self.heaps:
            self.words_in_heaps[heap] = self.full_player_data[status == heap]
    ...
    def update_game(self):
        ...
        if self.current_state == 'none selected':
            ...
            self.available_stakes = self.full_scoring_info[self.full_scoring_info.Stapel == self.current_heap].Inzet.unique()
            self.selected_article = ''
            self.selected_stake = ''
    ...
    def update_ui(self):
        ...
        if self.current_state in ('start', 'preparing'):
    ...
    def set_prospective_score(self): 
        scoring_for_heap = self.full_scoring_info[self.full_scoring_info.Stapel == self.current_heap]
